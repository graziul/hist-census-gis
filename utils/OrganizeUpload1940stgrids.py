...
df1['map_path'] = df1['map_path'].apply(lambda x: x.replace("Z:\\","S:\\"))+"\\"
df1['map_path'] = df1['map_path'].apply(lambda x: x.replace("S:\\S4\\","S:\\"))+"\\"
del df1['index']
df1['city_name'],df1['state_abbr'] = df1['citystate'].str.split(',', 1).str
df1['city_name'] = df1['city_name'].str.replace('.','')
df1['state_abbr'] = df1['state_abbr'].str.replace(' ','')
del df1['citystate']
#Save relevant information for Clean.py (i.e. the microdata cleaning algorithm)
df2 = df1[['city_name','state_abbr']]
...
def copy_rename_map_files(map_path, map_name, city, state):
	...
	if len(map_files) == 0:
		print("No files found for %s" % (city))
		raise SystemExit
	for f in map_files:
		if ~os.path.isfile(f):
			win32api.SetFileAttributes(f,win32con.FILE_ATTRIBUTE_READONLY)
			new_file_name = city.replace(' ','')+state+"_1940_stgrid_edit"
			new_f = dir_path+f.split("\\")[-1].replace(map_name,new_file_name)
			copyfile(f, new_f)
	return None

t = df1.apply(lambda x: copy_rename_map_files(x['map_path'],x['map_name'],x['city_name'],x['state_abbr']),axis=1)
