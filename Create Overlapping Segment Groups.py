...
for name in ["StLouis"]:
    ...
    ExtSegGr_grid = "Z:\Projects\\1940Census\\" + name + "\GIS_edited\\" + name + "_ExSegGr.shp"            # This is Extented Segment Group File
    ...
    Out_Join_c="Z:\Projects\\1940Census\\" + name + "\GIS_edited\\" + name + "_Join_c.shp"
    Out_Join_Layer="Z:\Projects\\1940Census\\" + name + "\GIS_edited\\" + name + "_Out_Join.lyr"
 ...
 #   arcpy.CalculateField_management(target_features, "Seg_ID", expression_Seg_ID, "PYTHON_9.3")
    ...
    arcpy.MakeFeatureLayer_management(Out_Join, Out_Join_Layer, "", "", fieldinfo)
    #Join table to layer
    arcpy.AddJoin_management(Out_Join_Layer, "UniqID", Out_Join_2, "UniqID", "KEEP_COMMON")
    #Create new shapefile to be used in Dissolve Procedure
    arcpy.CopyFeatures_management(Out_Join_Layer, Out_Join_c)

    print "Starting Dissolve Procedure"
    
    #Dissolve Join Result
    arcpy.Dissolve_management(Out_Join_c, ExtSegGr_grid, "SegGrID_1")
    ...
    arcpy.Delete_management(Out_Join_c)
    arcpy.Delete_management(Out_Join_Layer)
