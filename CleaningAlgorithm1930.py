...
def clean_microdata(city):
    ...
    cprint('%s Automated Cleaning\n' % (city_name), attrs=['bold'], file=sys.stdout)
    ...
    df = pd.read_stata(file_path + '/%s.dta' % (file_name))
    ...
    cprint('Loading data for %s took %s' % (city_name,load_time), 'cyan', attrs=['dark'], file=sys.stdout)
    ...
    cprint('Precleaning street names for %s took %s\n' % (city_name,preclean_time), 'cyan', attrs=['dark'], file=sys.stdout)
    ...
    cprint("Exact matching algorithm\n",attrs=['underline'],file=sys.stdout)
    ...
    cprint("Exact matches before ED validation: "+str(sm_exact_matches)+" of "+str(num_records)+" cases ("+str(round(100*float(sm_exact_matches)/float(num_records),1))+"%)",file=sys.stdout)
    ...
    cprint("Failed ED validation: "+str(num_failed_validation)+" of "+str(sm_exact_matches)+" cases with exact name matches ("+str(round(100*float(len(failed_validation))/float(sm_exact_matches),1))+"%)",'red',file=sys.stdout)
    cprint("Exact matches after ED validation: "+str(num_passed_validation)+" of "+str(num_records)+" cases ("+str(round(100*float(num_passed_validation)/float(num_records),1))+"%)",file=sys.stdout)
    ...
    cprint("Exact matching and ED validation for %s took %s\n" % (city_name,exact_matching_time), 'cyan', attrs=['dark'], file=sys.stdout)
    ...
    num_residual_cases = num_records - num_passed_validation

    cprint("Fuzzy matches (using microdata ED): "+str(sm_fuzzy_matches)+" of "+str(num_residual_cases)+" unmatched cases ("+str(round(100*float(sm_fuzzy_matches)/float(num_residual_cases),1))+"%)\n",file=sys.stdout)
    cprint("Fuzzy matching for %s took %s\n" % (city_name,fuzzy_matching_time),'cyan',attrs=['dark'],file=sys.stdout)
    ...
    cprint("Total processing time for %s: %s\n" % (city_name,total_time),'cyan',attrs=['bold'],file=sys.stdout)
    sys.stdout.close()

    df.to_csv(file_path + '/autocleaned/%s_AutoCleaned.csv' % (file_name))

    return [year,city_name,num_records, 
    num_passed_validation, float(num_passed_validation/num_records),
    sm_fuzzy_matches, float(sm_fuzzy_matches/num_records),
    sm_st_overall_matches, float(sm_st_overall_matches/num_records),
    num_failed_validation, float(num_failed_validation/num_records),
    num_records-sm_st_overall_matches, float((num_records-sm_st_overall_matches)/num_records),
    num_pairs_failed_validation, 
    num_pairs, 
    load_time, preclean_time, exact_matching_time, fuzzy_matching_time, total_time] 

year = 1930

file_path = '/home/s4-data/LatestCities/%s' % (year)
...
dashboard.to_csv(file_path + '/CleaningSummary%s.csv' % (str(year)))
